...
///
/// The class is marked `@unchecked Sendable` and the closure
/// type is `@Sendable` so this adapter compiles cleanly under
/// Swift 6 strict concurrency. `@unchecked` (rather than
/// derived `Sendable`) is necessary because the parent
/// `FfiMlsKeyProvider` protocol generated by UniFFI 0.28 isn't
/// itself `Sendable`-annotated, so Swift can't auto-derive
/// `Sendable` here. The class IS structurally safe — every
/// stored property is an immutable `let` binding, and the two
/// closures carry the `@Sendable` requirement that they only
/// capture thread-safe state (the platform shell's MLS
/// exporter, which is thread-safe by trait contract). When
/// UniFFI gains Swift 6 concurrency support, the `@unchecked`
/// escape hatch can be dropped.
public final class ClosureMlsKeyProvider: FfiMlsKeyProvider, @unchecked Sendable {
    public typealias SecretProducer = @Sendable (String) throws -> Data
